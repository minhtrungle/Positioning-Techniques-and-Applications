...
if (isNewRun)
fseek(file,0,-1);
[gpsdata, scount] = fread(file,50000,'schar');  % read approx. 3 mseconds of data
%gpsdata = gpsdata'-128;

%skip through that data file to start at the appropriate sample (corresponding to code phase)
fseek(file,(mycodephase-1),'bof');
...
while (loopcnt <=  msec)
    ...
    if (rem(loopcnt,100)==0)
        disp(['   Currently on interation:',int2str(loopcnt),' of:',int2str(msec)])
    end
    ...
    [rawdata,scount] = fread(file,blksize,'schar');
    %rawdata=rawdata-128;
